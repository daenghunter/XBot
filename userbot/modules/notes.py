...
async def notes_active(svd):
    ...
    message = "`Tidak ada catatan disimpan disini bro`"
...
async def remove_notes(clr):
    ...
    if rm_note(clr.chat_id, notename) is False:
        return await clr.edit("`Tidak menemukan Catatan Bro.. :` **{}**".format(notename))
    else:
        return await clr.edit(
            "`Berhasil menghapus catatan bro..:` **{}**".format(notename))
    ...
    if msg and msg.media and not string:
        if BOTLOG_CHATID:
            await fltr.client.send_message(
                ...
                "\n\n`Untuk mengikuti catatan yang anda kirimkan, tolong jangan dihapus catatan ini !`"
            )
...
async def kick_marie_notes(kick):
    ...
    await kick.edit("`Saya akan menghapus semua catatan`")
    await sleep(3)
    ...
    for i in filters:
        ...
        await sleep(0.3)
    await kick.respond(
        "```Berhasil Purge Catatan..```")
    if BOTLOG:
        await kick.client.send_message(
            BOTLOG_CHATID, "Saya menghapus semua catatan di " + str(kick.chat_id))
